...
export async function GET(
  ...
  if (verification.status === "processing") {
    const checkTime = new Date(verification.updated_at || verification.created_at).getTime();
    const now = Date.now();
    ...
    if (now - checkTime > STALE_TIMEOUT_MS) {
      await supabase
        ...
        .update({
          status: "error",
          error_message: "Обработка заняла слишком долго. Попробуйте загрузить смету заново.",
        })
        .eq("id", id);
      verification.status = "error";
      verification.error_message = "Обработка заняла слишком долго. Попробуйте загрузить смету заново.";
    }
  }
